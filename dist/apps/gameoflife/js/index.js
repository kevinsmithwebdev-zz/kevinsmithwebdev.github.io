...
var Game = function (_React$Component5) {
  ...
  function Game() {
    ...
    _this5.state = {
      ...
      turn: 0,
      color: "rainbow",
      presets: [{ name: "Glider", shape: [[-1, -1], [0, -1], [1, -1], [-1, 0], [0, 1]]
  ...
  Game.prototype.handleRandom = function handleRandom() {
    ...
    for (var i = 0; i < newLife.length; i++) {
      if (Math.random() < PROB && newLife[i] == 0) newLife[i] = 1;
    }
  ...
  Game.prototype.handleStart = function handleStart() {
    if (this.state.isRunning) {
      ...
      // this.getCoordList(); // just for getting coords for presets
  ...
  Game.prototype.handlePreset = function handlePreset(id) {
    ...
    for (var i = 0; i < this.state.presets[id].shape.length; i++) {
      newLife[this.getId(midX + this.state.presets[id].shape[i][0], midY + this.state.presets[id].shape[i][1])] = 1;
    }
  ...
  Game.prototype.render = function render() {
    ...
    return React.createElement(
      ...
      React.createElement("br", null),
      React.createElement(Controls, { isRunning: this.state.isRunning, color: this.state.color, speed: this.state.speed, presets: this.state.presets,
